...
class Penjualan extends Model
{
    ...
    public static function inputPenjualan($id_customer, $id_pengajuan, $dana_pengajuan)
    {
        ...
        // ['no_transaksi','id_customer','tgl_transaksi','tgl_expired','total_pengajuan','status'];
        if ($jml == 0) {
            ...
            DB::table('penjualan')->insert([
                ...
                'total_pengajuan' => $dana_pengajuan,
            ...
            if ($date > $mak_expired) {
                ...
                // cek untuk id pengajuan yang sama, maka tidak usah tambah lagi, 
                // tapi cukup jml pengajuannya diupdate
                // selain itu masukkan lagi ke penjualan detail
                // 1. cek apakah yg diinputkan adalah id pengajuan yang sudah ada di keranjang atau tidak
                $sql = "SELECT id_pengajuan, dana_pengajuan, no_transaksi FROM penjualan_detail
                    WHERE  
                    no_transaksi IN 
                    (
                        SELECT no_transaksi
                    ...
                    ";
                $barang = DB::select($sql, [$id_customer, $id_pengajuan]);
                $cek = 0;
                foreach ($barang as $b) :
                    $id_pengajuan_tabel = $b->id_pengajuan;
                    $dana_pengajuan_tabel = $b->dana_pengajuan;
                    $no_transaksi_tabel = $b->no_transaksi;
                    $cek = 1;
                    // tambahkan dana pengajuannya dan tambahkan masa expirednya
                    $date_plus_3 = Date('Y-m-d H:i:s', strtotime('+3 days')); //tambahkan 3 hari untuk expired datenya
                    $dana_pengajuan_akhir = $dana_pengajuan + $dana_pengajuan_tabel;
                    $total_pengajuan  = $dana_pengajuan * $dana_pengajuan_akhir;
                    $affected = DB::table('penjualan_detail')
                        ->where('no_transaksi', '=', $no_transaksi_tabel)
                        ->where('id_pengajuan', '=', $id_pengajuan_tabel)
                        ->update([
                            'dana_pengajuan' => $dana_pengajuan_akhir, 
                ...
                if ($cek == 0) {
                    ...
                    $sql = "SELECT total_pengajuan  FROM penjualan
                        WHERE no_transaksi = ? 
                       ";
                    $barang = DB::select($sql, [$no_transaksi]);
                    foreach ($barang as $b) :
                        $total_pengajuan_lama = $b->total_pengajuan;
                    endforeach;

                    // $total_pengajuan_lama = $b->total_pengajuan;
                    // masukkan ke tabel induk dulu yaitu di tabel penjualan
                    $total_pengajuan_baru = $dana_pengajuan + $total_pengajuan_lama;
                    $date = date('Y-m-d H:i:s');
                    $date_plus_3 = Date('Y-m-d H:i:s', strtotime('+3 days')); //tambahkan 3 hari untuk expired datenya
                    // update total pengajuan di penjualan karena sudah ditambah item baru
                    $affected = DB::table('penjualan')
                        ->where('no_transaksi', $no_transaksi)
                        ->update(
                            [
                                'tgl_expired' => $date_plus_3,
                                'total_pengajuan' => $total_pengajuan_baru,
                            ]
                        );
    ...
    public static function viewKeranjang($id_customer, $jenisKegiatan)
    {
    ...
    // view keranjang belanja
    public static function viewSiapBayar($id_customer, $jenisDana)
    {
        $sql = "SELECT  a.no_transaksi,
                        c.kode_$jenisDana,
                        b.tgl_transaksi,
